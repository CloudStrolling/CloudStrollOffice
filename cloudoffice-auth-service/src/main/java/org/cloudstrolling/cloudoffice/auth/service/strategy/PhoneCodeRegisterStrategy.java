 ...
 * 手机+验证码注册策略。
 *
 * <p>处理 PHONE_CODE 模式的注册流程：校验手机+验证码 → 自动生成 loginName
 * → 创建用户（accountSetled=false） → 分配默认角色 → 自动登录签发 Token 对。</p>
 *
...
public class PhoneCodeRegisterStrategy implements RegisterStrategy {
    ...
    public RegisterResult register(RegisterRequest request) {
        ...
        log.info("手机验证码注册开始 | phone={} | tenantCode={}", request.getPhone(), request.getTenantCode());
        ...
        UserEntity user = buildUser(request, tenantId, loginName, false);
        ...
        log.info("手机验证码注册完成 | userId={} | loginName={}", user.getId(), user.getLoginName());
        return result;
    }
    ...
    private UserEntity buildUser(RegisterRequest request, Long tenantId,
                                 String loginName, boolean accountSettled) {
        ...
        user.setRegisterMode("PHONE_CODE");
