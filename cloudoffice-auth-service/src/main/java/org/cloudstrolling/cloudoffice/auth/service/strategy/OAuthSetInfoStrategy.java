...
/**
 * OAuth 注册后设置信息策略（两步注册第一步）。
 *
 * <p>处理 OAUTH_SET_INFO 模式的注册流程：校验 oauthProvider+oauthCode
 * → 校验 openId 全局唯一 → 创建用户（accountSetled=false）
 * → 创建 OAuth 绑定记录 → 分配默认角色 → 自动登录签发 Token 对。</p>
 *
...
public class OAuthSetInfoStrategy implements RegisterStrategy {
    ...
    public RegisterResult register(RegisterRequest request) {
        ...
        log.info("OAuth 注册后设置信息开始 | oauthProvider={} | oauthCode={} | tenantCode={}",
        ...
        log.info("OAuth 注册后设置信息完成 | userId={} | loginName={}", user.getId(), user.getLoginName());
        return result;
    }
    ...
    private UserEntity buildUser(RegisterRequest request, Long tenantId, String loginName) {
        ...
        user.setRegisterMode("OAUTH_SET_INFO");
