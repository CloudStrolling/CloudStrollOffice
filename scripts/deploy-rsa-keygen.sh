...
echo "=============================================="
echo ""
echo "环境变量设置（将以下 Base64 值分别设置为环境变量）："
echo ""
echo "export RSA_PRIVATE_KEY=\"$(cat $PRIVATE_KEY_B64_FILE)\""
echo "export RSA_PUBLIC_KEY=\"$(cat $PUBLIC_KEY_B64_FILE)\""
echo ""
echo "或保存到文件供启动脚本读取："
echo "  $PRIVATE_KEY_B64_FILE"
echo "  $PUBLIC_KEY_B64_FILE"
echo ""

# 生成适用于 .env 文件的格式
echo "===== .env 文件片段（可复制到 .env 文件中）====="
