...
# ============================================================
# deploy-start-auth.sh - Auth Service 服务启动脚本 (Bash)
# 版本: v0.2.7
# 说明:
#   基于 deploy/env.json（经 load-env 统一加载，F-001）单服务启动认证服务（F-009）：
#     1. 前置校验：JDK 可用 + jar 包存在 + 本服务关键环境变量就绪
#        （任一缺失输出缺失项与处理提示，以非零码退出且不启动服务）
#     2. 后台启动：java -Xms256m -Xmx512m -jar <jar>
#        （nohup 忽略挂断信号后台运行，日志落位 deploy/logs/auth-start.log，
#        PID 记录 deploy/logs/auth.pid）
#     3. 健康确认：HTTP 直连自身端口（http://localhost:9100/api/v1/auth/health），端口探测备用；
#        循环轮询（可配置重试次数/间隔/单次超时，默认 30 次/2 秒/3 秒）
#     4. 任一步骤失败即停：输出明确错误提示，退出非零
#   行为与 deploy-start-all.sh 中 auth 服务启动逻辑一致（F-008/F-009）。
#   输出分级（通过/警告/失败）与退出码约定（F-011）：全部通过退出 0；存在失败项退出 1。
#   安全约定：口令/密钥不打印明文（RSA 密钥对 / DB_PASSWORD 仅校验非空，缺失提示只列键名）。
#   可配置环境变量：RETRY_COUNT（默认 30）、RETRY_INTERVAL（默认 2）、PROBE_TIMEOUT（默认 3）。
# 用法: ./deploy/scripts/deploy-start-auth.sh
# ============================================================
...
# ========== 2. 本服务契约（与 deploy-start-all.sh 中 auth 子块一致，F-009） ==========
# 服务标识（用于日志/PID 命名）与 jar 文件名
SERVICE_NAME="auth"
JAR_NAME="cloudoffice-auth-service.jar"
SERVICE_PORT=9100
HEALTH_URL="http://localhost:9100/api/v1/auth/health"
# 本服务关键环境变量（F-009 契约表）：缺失只列键名，不打印值
# 注意：auth-service 使用 DB_USERNAME（区别于 biz-service 的 DB_USER，差异保持现状）；
#       DB_USERNAME 已由 load-env 的 8 项关键配置兜底校验，本脚本按契约表不再重复校验。
REQUIRED_VARS=(NACOS_ADDR RSA_PUBLIC_KEY RSA_PRIVATE_KEY DB_PASSWORD)
MISSING_HINT="请检查 RSA 密钥对/DB_PASSWORD 配置"

# ========== 3. 标题 ==========
echo ""
echo "=============================================="
echo "  云漫智企 (CloudStrollOffice) Auth Service 服务启动"
echo "  版本: v0.2.7"
...
# ========== 5. 后台启动 + 健康确认（与 deploy-start-all.sh 中 auth 子块逻辑一致，F-009） ==========
...
echo ""
echo "=============================================="
echo -e "  Auth Service 服务启动完成: ${GREEN}通过 $PASS 项${NC} | ${YELLOW}警告 $WARN 项${NC} | ${RED}失败 $FAIL 项${NC}"
echo "=============================================="
echo ""
if [ "$FAIL" -gt 0 ]; then
  ...
  echo -e "${GREEN}Auth Service 服务启动成功且健康确认通过。${NC}"
  exit 0
fi
