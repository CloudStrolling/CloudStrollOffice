...
.DESCRIPTION
  基于 deploy/env.json（经 load-env 统一加载，F-001）单服务启动 API 网关（F-009）：
    ...
    2. 后台启动：java -Xms256m -Xmx512m -jar <jar>
       （Start-Process 隐藏窗口后台运行，日志落位 deploy/logs/gateway-start.log/.err，
  ...
  行为与 deploy-start-all 中 gateway 服务启动逻辑一致（F-008/F-009）。
...
.EXAMPLE
  .\deploy\scripts\deploy-start-gateway.ps1 -RetryCount 60 -RetryInterval 2 -ProbeTimeout 3
#>
param(
...
# ========== 2. 本服务契约（与 deploy-start-all 中 gateway 子块一致，F-009） ==========
# 服务标识（用于日志/PID 命名）与 jar 文件名
$ServiceName = "gateway"
$JarName     = "cloudoffice-gateway.jar"
$ServicePort = 9000
$HealthUrl   = "http://localhost:9000/"
# 本服务关键环境变量（F-009 契约表）：缺失只列键名，不打印值
$RequiredVars = @("NACOS_ADDR", "RSA_PUBLIC_KEY")
$MissingHint  = "请检查 NACOS_ADDR/RSA_PUBLIC_KEY 配置"

# ========== 3. 标题 ==========
Write-Host ""
Write-Host "=============================================="
Write-Host "  云漫智企 (CloudStrollOffice) Gateway 服务启动"
...
# ========== 5. 后台启动 + 健康确认（与 deploy-start-all 中 gateway 子块逻辑一致，F-009） ==========
...
Write-Host ""
Write-Host "=============================================="
Write-Host "  Gateway 服务启动完成: 通过 $script:pass 项 | 警告 $script:warn 项 | 失败 $script:fail 项"
Write-Host "=============================================="
Write-Host ""
if ($script:fail -gt 0) {
  ...
  Write-Host "Gateway 服务启动成功且健康确认通过。" -ForegroundColor Green
  exit 0
}
