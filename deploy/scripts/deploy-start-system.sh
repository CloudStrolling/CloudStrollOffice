...
# ============================================================
# deploy-start-system.sh - System Service 服务启动脚本 (Bash)
# 版本: v0.2.7
# 说明:
...
#        （nohup 忽略挂断信号后台运行，日志落位 deploy/logs/system-start.log，
...
#   行为与 deploy-start-all.sh 中 system 服务启动逻辑一致（F-008/F-009）。
...
# 用法: ./deploy/scripts/deploy-start-system.sh
# ============================================================
...
# ========== 2. 本服务契约（与 deploy-start-all.sh 中 system 子块一致，F-009） ==========
# 服务标识（用于日志/PID 命名）与 jar 文件名
SERVICE_NAME="system"
JAR_NAME="cloudoffice-system-service.jar"
SERVICE_PORT=9400
HEALTH_URL="http://localhost:9400/api/v1/system/health"
...
echo ""
echo "=============================================="
echo "  云漫智企 (CloudStrollOffice) System Service 服务启动"
echo "  版本: v0.2.7"
...
# ========== 5. 后台启动 + 健康确认（与 deploy-start-all.sh 中 system 子块逻辑一致，F-009） ==========
...
echo ""
echo "=============================================="
echo -e "  System Service 服务启动完成: ${GREEN}通过 $PASS 项${NC} | ${YELLOW}警告 $WARN 项${NC} | ${RED}失败 $FAIL 项${NC}"
echo "=============================================="
echo ""
if [ "$FAIL" -gt 0 ]; then
  ...
  echo -e "${GREEN}System Service 服务启动成功且健康确认通过。${NC}"
  exit 0
fi
