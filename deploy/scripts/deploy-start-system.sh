...
# 版本: v0.1.7
# 说明: 自动注入环境变量并启动系统服务
# 前置条件: 已执行 source deploy/scripts/deploy-env-local.sh
...
JAR_PATH="$PROJECT_DIR/cloudoffice-system-service.jar"
...
if [ -z "${NACOS_ADDR:-}" ]; then
  echo "❌ 错误: NACOS_ADDR 未设置。请先配置 deploy 目录下的 env.json 文件"
  exit 1
fi

if [ -z "${DB_PASSWORD:-}" ]; then
  echo "❌ 错误: DB_PASSWORD 未设置。请先配置 deploy 目录下的 env.json 文件"
  exit 1
fi
