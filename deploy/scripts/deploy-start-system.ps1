...
<#
.SYNOPSIS
  云漫智企 (CloudStrollOffice) System Service 服务启动脚本 (Windows)
.DESCRIPTION
  基于 deploy/env.json（经 load-env 统一加载，F-001）单服务启动系统服务（F-009）：
    1. 前置校验：JDK 可用 + jar 包存在 + 本服务关键环境变量就绪
       （任一缺失输出缺失项与处理提示，以非零码退出且不启动服务）
    2. 后台启动：java -Xms256m -Xmx512m -jar <jar>
       （Start-Process 隐藏窗口后台运行，日志落位 deploy/logs/system-start.log/.err，
       PID 记录 deploy/logs/system.pid）
    3. 健康确认：HTTP 直连自身端口（http://localhost:9400/api/v1/system/health），端口探测备用；
       循环轮询（可配置重试次数/间隔/单次超时，默认 30 次/2 秒/3 秒）
    4. 任一步骤失败即停：输出明确错误提示，退出非零
  行为与 deploy-start-all 中 system 服务启动逻辑一致（F-008/F-009）。
...
.EXAMPLE
  .\deploy\scripts\deploy-start-system.ps1 -RetryCount 60 -RetryInterval 2 -ProbeTimeout 3
#>
param(
...
# ========== 2. 本服务契约（与 deploy-start-all 中 system 子块一致，F-009） ==========
# 服务标识（用于日志/PID 命名）与 jar 文件名
$ServiceName = "system"
$JarName     = "cloudoffice-system-service.jar"
$ServicePort = 9400
...
Write-Host ""
Write-Host "=============================================="
Write-Host "  云漫智企 (CloudStrollOffice) System Service 服务启动"
...
# ========== 5. 后台启动 + 健康确认（与 deploy-start-all 中 system 子块逻辑一致，F-009） ==========
...
Write-Host ""
Write-Host "=============================================="
Write-Host "  System Service 服务启动完成: 通过 $script:pass 项 | 警告 $script:warn 项 | 失败 $script:fail 项"
Write-Host "=============================================="
Write-Host ""
if ($script:fail -gt 0) {
  ...
  Write-Host "System Service 服务启动成功且健康确认通过。" -ForegroundColor Green
  exit 0
}
