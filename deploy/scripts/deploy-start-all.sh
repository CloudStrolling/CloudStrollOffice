...
echo ""
print_result "通过" "前置校验：4 个 jar 包与关键环境变量全部就绪"
...
for entry in "${SERVICES[@]}"; do
  ...
  if wait_health_up "$url" "$port" "$RETRY_COUNT" "$RETRY_INTERVAL" "$PROBE_TIMEOUT"; then
    ...
    print_result "失败" "$name: 健康确认超时（重试 $RETRY_COUNT 次/间隔 $RETRY_INTERVAL 秒后仍无响应）。可能原因：端口 $port 被占用（请检查 9000/9100/9200/9400）、服务启动失败或依赖未就绪；请查看日志 $LOG_FILE；$hint"
    break
  fi
done
