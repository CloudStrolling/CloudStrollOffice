...
<#
.SYNOPSIS
  云漫智企 (CloudStrollOffice) Common 服务启动脚本 (Windows)
.DESCRIPTION
  基于 deploy/env.json（经 load-env 统一加载，F-001）单服务启动 cloudoffice-common（v0.2.8 F-009）：
    1. 前置校验：JDK 可用 + jar 包存在 + 本服务关键环境变量就绪
       （任一缺失输出缺失项与处理提示，以非零码退出且不启动服务）
    2. 后台启动：java -Xms256m -Xmx512m -jar <jar>
       （Start-Process 隐藏窗口后台运行，日志落位 deploy/logs/common-start.log/.err，
       PID 记录 deploy/logs/common.pid）
    3. 健康确认：HTTP 直连自身端口（http://localhost:{port}/api/v1/common/health），端口探测备用；
       循环轮询（可配置重试次数/间隔/单次超时，默认 30 次/2 秒/3 秒）
    4. 任一步骤失败即停：输出明确错误提示，退出非零
  行为与 deploy-start-all 中 common 服务启动逻辑一致（F-008/F-009）。
  输出分级（通过/警告/失败）与退出码约定（F-011）：全部通过退出 0；存在失败项退出 1。
  安全约定：口令/密钥不打印明文（NACOS_ADDR / COMMON_PORT / DB_PASSWORD 仅校验非空，缺失提示只列键名）。
  版本: v0.2.8
.EXAMPLE
  .\deploy\scripts\deploy-start-common.ps1
.EXAMPLE
  .\deploy\scripts\deploy-start-common.ps1 -RetryCount 60 -RetryInterval 2 -ProbeTimeout 3
#>
param(
...
# ========== 2. 本服务契约（与 deploy-start-all 中 common 子块一致，v0.2.8 F-009） ==========
# 服务标识（用于日志/PID 命名）与 jar 文件名
$ServiceName = "common"
$JarName     = "cloudoffice-common.jar"
# common 端口读 COMMON_PORT 环境变量（TASK-009 加入 env.json；缺省 9300）
$ServicePort = "9300"
if (-not [string]::IsNullOrEmpty($env:COMMON_PORT)) { $ServicePort = $env:COMMON_PORT }
$HealthUrl   = "http://localhost:$ServicePort/api/v1/common/health"
...
Write-Host ""
Write-Host "=============================================="
Write-Host "  云漫智企 (CloudStrollOffice) Common 服务启动"
...
# ========== 5. 后台启动 + 健康确认（与 deploy-start-all 中 common 子块逻辑一致，F-009） ==========
...
Write-Host ""
Write-Host "=============================================="
Write-Host "  Common 服务启动完成: 通过 $script:pass 项 | 警告 $script:warn 项 | 失败 $script:fail 项"
Write-Host "=============================================="
Write-Host ""
if ($script:fail -gt 0) {
  ...
  Write-Host "Common 服务启动成功且健康确认通过。" -ForegroundColor Green
  exit 0
}
