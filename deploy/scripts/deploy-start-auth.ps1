...
<#
.SYNOPSIS
  云漫智企 (CloudStrollOffice) Auth Service 服务启动脚本 (Windows)
.DESCRIPTION
  基于 deploy/env.json（经 load-env 统一加载，F-001）单服务启动认证服务（F-009）：
    ...
    2. 后台启动：java -Xms256m -Xmx512m -jar <jar>
       （Start-Process 隐藏窗口后台运行，日志落位 deploy/logs/auth-start.log/.err，
  ...
  行为与 deploy-start-all 中 auth 服务启动逻辑一致（F-008/F-009）。
...
# ========== 2. 本服务契约（与 deploy-start-all 中 auth 子块一致，F-009） ==========
# 服务标识（用于日志/PID 命名）与 jar 文件名
$ServiceName = "auth"
$JarName     = "cloudoffice-auth-service.jar"
$ServicePort = 9100
...
$MissingHint  = "请检查 RSA 密钥对/DB_PASSWORD 配置"

# ========== 3. 标题 ==========
Write-Host ""
Write-Host "=============================================="
Write-Host "  云漫智企 (CloudStrollOffice) Auth Service 服务启动"
...
# ========== 5. 后台启动 + 健康确认（与 deploy-start-all 中 auth 子块逻辑一致，F-009） ==========
...
Write-Host ""
Write-Host "=============================================="
Write-Host "  Auth Service 服务启动完成: 通过 $script:pass 项 | 警告 $script:warn 项 | 失败 $script:fail 项"
Write-Host "=============================================="
Write-Host ""
if ($script:fail -gt 0) {
  ...
  Write-Host "Auth Service 服务启动成功且健康确认通过。" -ForegroundColor Green
  exit 0
}
