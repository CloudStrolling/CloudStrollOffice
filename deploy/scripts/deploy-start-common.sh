...
# ============================================================
# deploy-start-common.sh - Common 服务启动脚本 (Bash)
# 版本: v0.2.8
# 说明:
...
#        （nohup 忽略挂断信号后台运行，日志落位 deploy/logs/common-start.log，
...
#   行为与 deploy-start-all.sh 中 common 服务启动逻辑一致（F-008/F-009）。
...
# 用法: ./deploy/scripts/deploy-start-common.sh
# ============================================================
...
# ========== 2. 本服务契约（与 deploy-start-all.sh 中 common 子块一致，v0.2.8 F-009） ==========
...
SERVICE_PORT="$COMMON_PORT"
HEALTH_URL="http://localhost:$SERVICE_PORT/api/v1/common/health"
# 本服务关键环境变量（F-009 契约表）：缺失只列键名，不打印值
REQUIRED_VARS=(NACOS_ADDR COMMON_PORT DB_PASSWORD)
MISSING_HINT="请检查 COMMON_PORT/DB_PASSWORD 配置"

# ========== 3. 标题 ==========
echo ""
echo "=============================================="
echo "  云漫智企 (CloudStrollOffice) Common 服务启动"
echo "  版本: v0.2.8"
...
# ========== 5. 后台启动 + 健康确认（与 deploy-start-all.sh 中 common 子块逻辑一致，F-009） ==========
...
echo ""
echo "=============================================="
echo -e "  Common 服务启动完成: ${GREEN}通过 $PASS 项${NC} | ${YELLOW}警告 $WARN 项${NC} | ${RED}失败 $FAIL 项${NC}"
echo "=============================================="
echo ""
if [ "$FAIL" -gt 0 ]; then
  ...
  echo -e "${GREEN}Common 服务启动成功且健康确认通过。${NC}"
  exit 0
fi
