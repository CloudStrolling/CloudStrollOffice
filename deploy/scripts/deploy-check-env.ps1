...
.PARAMETER RedisPort
  Redis 端口 (默认: 6379)
.EXAMPLE
  .\deploy\scripts\deploy-check-env.ps1 -DbPassword "YourP@ss"
#>
...
Check -Desc "项目代码已就绪" -Expected "pom.xml 文件存在" -Cmd {
  if (Test-Path (Join-Path $RootDir "pom.xml")) { return $true } else { throw "pom.xml 不存在" }
}

Check -Desc "SQL 初始化脚本存在" -Expected "scripts/sql/ 目录下存在 SQL 文件" -Cmd {
  $f1 = Test-Path (Join-Path $RootDir "scripts\sql\auth-init-v0.1.5.sql")
  $f2 = Test-Path (Join-Path $RootDir "scripts\sql\auth-init-v0.1.6.sql")
