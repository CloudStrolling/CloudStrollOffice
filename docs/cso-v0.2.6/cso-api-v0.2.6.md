...
以上变更均为**构建/依赖配置与密钥格式契约类修复**，**不触碰接口层（Controller/DTO/响应体）**，对外接口契约 **API-001~API-033 完整保留，客户端运行时代码零改动**（PRD v0.2.6 F-005 与验收标准第 6 条）。本版本接口设计完全沿用主文档 `docs/cso-api.md`（v0.0.1 基线，v0.2.5 确认无变更后继续沿用）。
...
**本版本（v0.2.6）无新增接口**，API-001~API-033 的接口详细定义（请求头/请求参数/请求示例/响应参数/响应示例/错误码）**完全沿用主文档 `docs/cso-api.md`（v0.0.1）第 5 章**，本版本不做重复编写与任何变更。
