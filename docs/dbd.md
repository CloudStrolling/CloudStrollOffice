...
| 云服务 | `cloudstroll_office_cloud` | 云资源管理，云主机、存储、网络编排 |
...
## 3.3 云服务数据库 —— `cloudstroll_office_cloud`

v0.1.0 骨架阶段为预留状态，以下为后续版本的预期表结构设计。

### 3.3.1 t_cloud_resource（云资源表）

| 字段名 | 类型 | 长度/精度 | 可空 | 默认值 | 约束 | 说明 |
|--------|------|----------|------|--------|------|------|
| id | BIGINT | 20 | NO | - | PK | 主键 ID（雪花算法） |
| enterprise_id | BIGINT | 20 | NO | - | None | 所属企业 ID |
| resource_type | VARCHAR | 50 | NO | - | None | 资源类型：vm-云主机 storage-存储 network-网络 |
| resource_name | VARCHAR | 200 | NO | - | None | 资源名称 |
| resource_spec | VARCHAR | 200 | YES | NULL | None | 资源规格（如 4C8G100G） |
| status | VARCHAR | 30 | NO | - | None | 资源状态：running-运行中 stopped-已停止 error-异常 |
| region | VARCHAR | 100 | YES | NULL | None | 所在区域/机房 |
| expire_time | DATETIME | 3 | YES | NULL | None | 到期时间 |
| create_time | DATETIME | 3 | NO | CURRENT_TIMESTAMP | None | 创建时间 |
| update_time | DATETIME | 3 | NO | CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP | None | 更新时间 |
| deleted | TINYINT | 1 | NO | 0 | None | 逻辑删除标记 |

**索引设计：**
- `idx_cloud_resource_enterprise` (`enterprise_id`) —— 按企业查询资源
- `idx_cloud_resource_type` (`resource_type`) —— 按资源类型查询
- `idx_cloud_resource_status` (`status`) —— 按状态查询

### 3.3.2 预留表（后续版本）

| 表名 | 预期用途 | 计划版本 |
|------|---------|---------|
| `t_cloud_instance` | 云主机实例表 | v0.3.0 |
| `t_cloud_storage` | 云存储表 | v0.3.0 |
| `t_cloud_network` | 网络资源表 | v0.3.0 |
| `t_cloud_order` | 云资源订单表 | v0.4.0 |

---

## 3.4 系统服务数据库 —— `cloudstroll_office_system`

v0.1.0 骨架阶段为预留状态，以下为后续版本的预期表结构设计。

### 3.4.1 t_sys_config（系统配置表）
...
### 3.4.3 t_sys_oper_log（操作日志表）

| 字段名 | 类型 | 长度/精度 | 可空 | 默认值 | 约束 | 说明 |
|--------|------|----------|------|--------|------|------|
| id | BIGINT | 20 | NO | - | PK | 主键 ID（雪花算法） |
| module | VARCHAR | 50 | NO | - | None | 操作模块（auth/biz/cloud/system） |
...
## 4.8 云资源类型（t_cloud_resource.resource_type）

| 字段 | 枚举值 | 含义 |
|------|--------|------|
| `t_cloud_resource.resource_type` | vm | 云主机 |
| `t_cloud_resource.resource_type` | storage | 云存储 |
| `t_cloud_resource.resource_type` | network | 网络资源 |

## 4.9 云资源状态（t_cloud_resource.status）

| 字段 | 枚举值 | 含义 |
|------|--------|------|
| `t_cloud_resource.status` | running | 运行中 |
| `t_cloud_resource.status` | stopped | 已停止 |
| `t_cloud_resource.status` | error | 异常 |

## 4.10 操作日志状态（t_sys_oper_log.status）
...
| 云服务 | `cloudstroll_office_cloud` | `cloud_app` | 仅授予 cloud 库权限 |
