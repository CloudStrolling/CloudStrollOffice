-- ============================================================
-- CloudStrollOffice（云漫智企）数据库设计文档配套 SQL 脚本
-- 版本：v0.0.1（初始化归档版本，对应实际业务版本 v0.1.6）
-- 生成日期：2026-08-05
...
--
-- 说明：
--   本脚本为数据库初始化全量脚本，整合认证服务（auth-service）的
--   全部 9 张业务表及初始数据，并预留企业服务（biz-service）与
--   系统服务（system-service）的数据库骨架（当前为骨架阶段，无业务表）。
--   建表使用 CREATE TABLE IF NOT EXISTS、初始数据使用 INSERT IGNORE，
--   脚本可重复执行（幂等），不会破坏已有数据。
--
-- 使用方式：
--   方式一：直接执行本脚本（需 CREATE DATABASE 权限）
--     mysql -u root -p < cso-dbd-v0.0.1.sql
--   方式二：手动创建数据库后按需执行建表语句
--
-- 安全提示：
--   生产环境请勿使用 DROP DATABASE，建议手动创建数据库后逐段执行。
--   管理员初始密码为 admin123（BCrypt 哈希），首次登录后请立即修改。
-- ============================================================
...
-- 说明：存储用户认证、授权、RBAC 多租户权限模型、OAuth 绑定、
--       验证码记录、登录日志等数据（共 9 张表）
-- 模块：cloudoffice-auth-service（端口 9100）
-- 对应功能：F-001 ~ F-012（多模式登录注册/RBAC/密码/手机号/验证码/日志）
-- ============================================================
...
-- 说明：存储 SaaS 平台企业租户信息，支持状态控制与过期管理，
--       是数据隔离边界（租户间数据不可见）
-- ------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `t_auth_user` (
    ...
    `lock_reason`               VARCHAR(255) DEFAULT NULL                COMMENT '锁定/封禁原因',
...
CREATE TABLE IF NOT EXISTS `t_auth_oauth_account` (
    ...
    `oauth_provider` VARCHAR(32)  NOT NULL                    COMMENT 'OAuth提供商（WECHAT/DINGTALK/WECHAT_WORK/ALIPAY 等）',
    ...
    `update_time`    DATETIME(3)  NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间（毫秒精度）',
    `deleted`        TINYINT(1)   NOT NULL DEFAULT 0          COMMENT '逻辑删除：0-正常 1-删除',
    PRIMARY KEY (`id`) USING BTREE,
...
CREATE TABLE IF NOT EXISTS `t_auth_verification_code` (
    ...
    `send_count`  INT(11)      DEFAULT 0                   COMMENT '已发送次数（用于频率控制）',
    `create_time` DATETIME(3)  NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间（毫秒精度）',
    `update_time` DATETIME(3)  NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间（毫秒精度）',
    `deleted`     TINYINT(1)   NOT NULL DEFAULT 0          COMMENT '逻辑删除：0-正常 1-删除',
    PRIMARY KEY (`id`) USING BTREE,
...
-- 第三部分：初始数据（Seed Data）
-- 说明：插入系统运行必需的初始数据
--       以下使用硬编码 ID（雪花算法），开发环境以 1 起始；
--       使用 INSERT IGNORE 确保重复执行时不会报错（幂等）
-- ============================================================
