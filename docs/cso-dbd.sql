...
--
-- 说明：
--   本脚本为全量初始化脚本，整合认证服务（auth-service）的
--   全部 9 张业务表及其初始数据，以及企业服务（biz-service）、
--   系统服务（system-service）的数据库骨架（仅建库不建表）。
--   脚本可重复执行（幂等），不会破坏已有数据。
--
-- 使用方式：
...
--
-- 安全提示：
--   生产环境请勿使用 DROP DATABASE 语句，建议手动创建数据库后，
--   逐段执行建表和初始化数据。
-- ============================================================
...
-- 清空并重建数据库（仅开发/测试环境使用，生产环境请注释掉以下 DROP 语句）
...
-- 关联 UserStory：US-006
-- 说明：register_mode/account_settled/phone_verified/email_verified/
--       last_password_change_time 为 v0.1.6 扩展字段
-- ------------------------------------------------------------
