 ...
 * Biz 服务健康检查控制器测试。
 *
 * <p>验证 /api/v1/biz/health 端点的正确响应。
 * 使用纯单元测试方式直接测试控制器逻辑，避免完整 Spring 上下文加载。</p>
 *
 * @author CloudStrolling
 * @since 1.0
 */
@DisplayName("Biz HealthController 测试")
class HealthControllerTest {
    ...
    void setUp() throws Exception {
        ...
        when(mockEnv.getProperty("spring.application.name", "cloudoffice-biz-service"))
                .thenReturn("cloudoffice-biz-service");
    ...
    @Test
    @DisplayName("GET /api/v1/biz/health -> 200 + ApiResult + 健康信息")
    void health_shouldReturn200AndApiResult_whenCalled() {
        ...
        assertEquals("cloudoffice-biz-service", data.get("service"), "service 名称应正确");
